...
object ImgUtils: CoroutineScope{
    ...
    suspend fun buildImageMessage(
        ...
        BufferedImage(imgWidth, height, BufferedImage.TYPE_INT_ARGB).apply {
            createGraphics().apply {
                setRenderingHints(ImgUtils.renderingHints)

                background(this, height, hex2Color(colorStr))
                val user = profile?.user!!
                val pendant = profile.pendant?.image ?: ""
                header(this, user.uname!!, time, user.face!!, pendant)

                var g2Y = 110
                biList.forEach {
                    drawImage(it, 0, g2Y, null)
                    g2Y += it.height
                }

                if (BiliPluginConfig.qrCode){
                    drawImage(qrContent(url, colorStr), 695, 15, null)
                }
            ...
            withContext(Dispatchers.IO) {
                ImageIO.write(this@apply, "png", file)
            }
            return file
        }
    }
    ...
    fun buildReplyImageMessage(biList: List<BufferedImage>, user: UserInfo?): BufferedImage {
        ...
        BufferedImage(imgWidth, height, BufferedImage.TYPE_INT_ARGB).apply bi@{
            createGraphics().apply {
                setRenderingHints(ImgUtils.renderingHints)

                color = Color(245, 245, 245)
                fillRoundRect(0, 0, imgWidth, height, 10, 10)

                var g2Y = if (user == null) 20 else 55
                if (user?.uname != null) {
                    if (user.face != null) {
                        clip = Ellipse2D.Double(25.0, 10.0, 40.0, 40.0)
                        drawImage(ImageIO.read(URL(imgApi(user.face, 40, 40))), 25, 10, null)
                        clip = null
                        stroke = BasicStroke(2f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND)
                        color = Color.WHITE
                        drawOval(25, 10, 40, 40)
                    }
                    color = Color(0, 161, 214)
                    font = fonts[0].deriveFont(25f)
                    drawString(user.uname, 80, 40)
                } else {
                    g2Y = 0
                }
            ...
            return BufferedImage(imgWidth, height, BufferedImage.TYPE_INT_ARGB).apply {
                createGraphics().apply {
                    setRenderingHints(ImgUtils.renderingHints)
                    drawImage(this@bi, 20, 20, imgWidth - 40, height * (imgWidth - 40) / imgWidth, null)
                    dispose()
                }
            }
        }
    ...
    suspend fun buildLiveImageMessage(
        ...
        BufferedImage(imgWidth, 570, BufferedImage.TYPE_INT_ARGB).apply bi@{
            createGraphics().apply {
                setRenderingHints(ImgUtils.renderingHints)

                background(this, 570, hex2Color(colorStr))
                header(this, title, "$uname    $time", faceUrl, "", -20, false)

                val margin = 10
                val rw = imgWidth - margin * 2
                val rh = 570 - margin * 2
                clip = RoundRectangle2D.Double(margin - 1.0, margin - 1.0, rw + 1.0, rh + 1.0, arc.toDouble(),
                    arc.toDouble()
                )
                drawImage(withContext(Dispatchers.IO) {
                    ImageIO.read(URL(imgApi(coverUrl, 780, 440)))
                }, 10, 120, 780, 440, null)
                //g2.drawImg(coverUrl, 10,120,780,440)

                val y = if (BiliPluginConfig.qrCode) 85 else 0
                color = Color(251, 114, 153)
                fillRoundRect(722, y + 50, 52, 27, 5, 5)
                color = Color.WHITE
                font = fonts[0].deriveFont(20f)
                drawString("直播", 728, y + 70)

                if (BiliPluginConfig.qrCode){
                    drawImage(qrContent(url, colorStr), 690, 20, null)
                }
    ...
    private fun background(bgG2: Graphics2D, height: Int, color: Color) {
        ...
        bgG2.fillRoundRect(margin, margin, rw, rh, arc, arc)
        bgG2.color = Color(238, 238, 238)
        bgG2.drawRoundRect(margin - 1, margin - 1, rw + 1, rh + 1, arc, arc)
    ...
    fun qrContent(url: String, color: String): BufferedImage {
        val qrCodeWriter = QRCodeWriter()
        val bitMatrix = qrCodeWriter.encode(url, BarcodeFormat.QR_CODE, 90, 90, mapOf(EncodeHintType.MARGIN to 0))
        ...
        val config = MatrixToImageConfig(ccc, 0x00FFFFFF)
    ...
    suspend fun articleContent(imageUrls: List<String>, title: String, desc: String): BufferedImage {
        ...
        articleG2.clip = RoundRectangle2D.Double(
            contentMargin.toDouble(), 10.0,
            cardWidth.toDouble(), cardHeight.toDouble(), picArc.toDouble(), picArc.toDouble()
        )
    ...
    suspend fun musicContent(coverUrl: String, title: String, desc: String, isMusic: Boolean = true): BufferedImage {
        ...
        musicG2.clip = RoundRectangle2D.Double(
            contentMargin.toDouble(), 10.0,
            cardWidth.toDouble(), cardHeight.toDouble(), picArc.toDouble(), picArc.toDouble()
        )
        ...
        if (isMusic) {
            ...
            val playImg = withContext(Dispatchers.IO) {
                ImageIO.read(ByteArrayInputStream(playImgByte))
            }
    ...
    private suspend fun downloadImg(url: String):BufferedImage? = try {
        withContext(Dispatchers.IO) {
            val input = client.get<InputStream>(url)

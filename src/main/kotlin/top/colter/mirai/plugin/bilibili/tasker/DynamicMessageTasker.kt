...
object DynamicMessageTasker : BiliTasker() {
    ...
    fun DynamicItem.dynamicLinks(): List<DynamicMessage.Link> {
        ...
        return when (type) {
            ...
            DYNAMIC_TYPE_ARTICLE -> {
                listOf(
                    DynamicMessage.Link(DYNAMIC_TYPE_ARTICLE.text, ARTICLE_LINK(this.modules.moduleDynamic.major?.article?.id!!)),
                    DynamicMessage.Link("动态", DYNAMIC_LINK(idStr))
                )
            }
            DYNAMIC_TYPE_AV -> {
                listOf(
                    DynamicMessage.Link(DYNAMIC_TYPE_AV.text, VIDEO_LINK(this.modules.moduleDynamic.major?.archive?.aid)),
                    DynamicMessage.Link("动态", DYNAMIC_LINK(idStr))
                )
            }
            DYNAMIC_TYPE_MUSIC -> {
                listOf(
                    DynamicMessage.Link(DYNAMIC_TYPE_MUSIC.text, MUSIC_LINK(this.modules.moduleDynamic.major?.music?.id!!)),
                    DynamicMessage.Link("动态", DYNAMIC_LINK(idStr))
                )
            }
            DYNAMIC_TYPE_PGC -> {
            ...
            DYNAMIC_TYPE_LIVE -> {
                listOf(
                    DynamicMessage.Link(DYNAMIC_TYPE_LIVE.text, LIVE_LINK(this.modules.moduleDynamic.major?.live?.id!!)),
                    DynamicMessage.Link("动态", DYNAMIC_LINK(idStr))
                )
            }
            DYNAMIC_TYPE_LIVE_RCMD -> {
